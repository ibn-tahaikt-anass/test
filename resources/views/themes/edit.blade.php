...
<body>
 ...
 <form action="{{ route('themes.update', $theme->id) }}" method="POST">
        @csrf
        @method('PUT')
 <label for="inputPassword">Modifier un Album</label>

 <input type="text" id="inputPassword" name="nom_theme" value="{{ $theme->nom_theme }}" required required>

 <button href="{{ route('home') }}"type="submit">Modifier</button>
